...
(2026-08-30 배포본 실측, 콜드 캐시, **N=5 교대 중앙값**. Edge 헤드리스+CDP 워터폴):

| | 앱 시작(DCL) | FCP | 총 바이트 |
|---|---|---|---|
| 데스크톱 | 477ms | 268ms | 369KB |
| 4G+CPU4배 | 781ms | 572ms | 369KB |

> 직전 기준선(2026-08-24)은 데스크톱 730/752ms · 4G 1,102/1,128ms · 384KB였다.
> FCP가 크게 내려온 것은 **폰트 CSS를 렌더 블로킹에서 뺀 것**이 원인이다(위 Pretendard 절).
> 인과 판정법(1회 측정으로도 가능): `measure_timeline.js`에서 **FCP가 폰트 조각 시작보다 앞**이면 적용된 것이다 —
> 고치기 전엔 FCP 1,268ms가 폰트 CSS 완료(1,174ms) *뒤*였고, 지금은 FCP 292ms가 폰트 조각 시작(448ms) *앞*이다.
> ⚠️ **개별 수치는 여전히 1회로 믿지 말 것** — 이 표를 낼 때도 같은 배포본에서 데스크톱 DCL이 351~1,595ms로 흔들렸다.
> 중앙값이라 표는 쓸 만하지만, `measure_load.js`의 시간 검사 FAIL은 한 번 더 돌려보고 판단할 것.
...
| | 629행(오늘) | 10,000행 | 40,000행 | 차수 | (개선 전 40,000행) |
|---|---|---|---|---|---|
| `bucketByPeriod`(6주기) | 0.13 | 1.5 | 11 | O(n) | 247 |
| `scoped()`(이번주기·전체멤버) | 0.08 | 0.6 | 5.0 | O(n) | 45 |
| `viewAnalysis` | 0.74 | 1.3 | 10 | O(n) | 130 |
| `render(내역)` 기본 화면 | 0.14 | 0.6 | 5.6 | O(n) | 58 |
| `viewCategory` | 0.08 | 0.6 | 5.3 | O(n) | 46 |
| `aggCat` · `taxCalc` · `orphanTransfers` | <0.2 | ~1 | ~4 | O(n) | 같음 |
| `listResultsHtml`(검색) | 1.3 | 8.2 | 35 | O(n) | 같음 |
| `saveSnapshot` / `readSnapshot` | 0.7 | 13 | 66 / — | O(n) | 같음 |

- **이차 복잡도는 없다**(629→40,000행 전 구간 확인). 40,000행에서도 폰 근사(×4)로 200ms를 넘는 항목이 없다.
- **2026-09-01 개선**: 주기 판정을 문자열 파싱에서 타임스탬프 비교로 바꿔 집계가 **11~22배** 빨라졌다.
  옛 `inPeriod(r.date, p)`는 행마다 주기 수만큼 `parseDate`(=`new Date(문자열)`)를 돌렸다 —
  40,000행·6주기면 14만 번. 지금은 `applyLoad`의 `stampRows()`가 행당 `_t`(숫자)를 한 번 심고
  `inPeriodRow(r,p)`가 `p.sT`·`p.eT`와 숫자로만 비교한다.
  - ⚠️ **`inPeriod(문자열, p)`로 되돌리지 말 것.** 결과는 같지만 비용이 돌아온다.
    문자열판은 `test_period.js`의 **참조 구현**으로만 남겨둔 것이다(둘이 같은 답을 내는지 대조).
  - ⚠️ `rowTime()`의 `_t` 폴백을 지우지 말 것 — 캐시 밖에서 만든 임시 행 객체가 조용히 전부 탈락한다
    (`test_period.js`가 이 변이로 6건 FAIL 한다).
  - ⚠️ `billingPeriod()`의 `sT`·`eT`는 `start`·`end`와 **같은 경계의 숫자판**이다. 한쪽만 고치면 갈린다.
- **스냅샷이 `SNAP_MAX`(2MB, ≈9,400행)를 넘으면** 포기하지 않고 **최근 45일만 담고 `partial:true`**를 단다.
  `bootFromSnapshot`이 그 표시를 보고 `ROWS_PARTIAL`을 켜므로 `needsFullRows()` 가드가 그대로 걸린다 —
  잘린 데이터로 잔액·공제액이 계산되는 일은 없다.
  ⚠️ 예전엔 그냥 `return false`였는데, 그러면 한도를 넘는 날부터 첫 화면이 다시 2~3초로 돌아가면서
  **화면에도 로그에도 단서가 없다**. 조용한 퇴화가 제일 나쁘다.
- `saveSnapshot`은 **마지막 렌더 뒤 idle에서** 부른다(40,000행 직렬화 66ms이 렌더 앞에 끼면 안 된다).
...
**전체 정의(컬럼·제약, `app_settings` 키 목록, upsert 계약 표, RLS SQL, 삭제된 뷰 경위) → [`docs/schema.md`](docs/schema.md)**
스키마 실물은 `mcp__supabase__list_tables` 로 실측 가능 — 문서와 어긋나면 DB가 정답이다.

테이블 6개: `members` · `transactions` · `category_limits` · `master_data` · `tax_map` · `app_settings`
⚠️ 같은 프로젝트의 `forests`·`rules`·`favorites`·`policy_snapshots` 는 **휴양림 앱 것**이다(위 '연결 정보' 참조).

### 어기면 조용히 깨지는 것들 (여기 남긴 이유)
- ⚠️ `transactions.type` CHECK 에 **`'이동'` 은 없다** — `saveTransfer()` 가 `지출`+`입금` 2건으로 풀고
  `category='계좌간 이동'` 으로 구분한다. '이동' 을 type 으로 저장하게 '단순화' 하면 insert 가 통째로 실패한다.
- ⚠️ `transactions.category`·`account` 는 NOT NULL — `saveEntry()` 의 필수 검증을 없애면 토스트 대신 DB 오류가 뜬다(둘은 같이 움직인다).
- ⚠️ `category_limits` 에 **`UNIQUE(category)` 단독 제약을 다시 만들지 말 것** (멤버별 한도를 막던 버그, 2026-06 제거).
- ⚠️ `tax_map` 의 설치 SQL 은 **앱 안에도 있다**(`public/index.html` 의 `TAX_MAP_DDL`). 스키마·RLS 를 고치면 **그쪽도 같이** 고칠 것 —
  2026-08-07 에 앱 DDL 만 옛 정책으로 남아, 실행됐다면 방금 닫은 구멍을 앱이 스스로 다시 열 뻔했다.
- ⚠️ RLS 는 **정책 + `revoke` 두 짝**이다. `to authenticated` 로 좁히고 `revoke` 를 빠뜨리면 anon 에게 `200 []` 가 가고,
  차단된 것처럼 보이지만 테이블 존재와 컬럼 구조는 그대로 샌다.
- ⚠️ **새 뷰는 반드시 `security_invoker=on`.** 옛 뷰 3개(`v_account_balance`·`v_limit_usage`·`v_period_category`)는
  RLS 를 우회해 anon 에게 잔액이 새던 경로라 2026-08-14 에 삭제됐다 — 이제 **404 가 정상**이고 `verify_rls.js` 가 재생성을 감시한다.
- ⚠️ **주기 집계의 유일한 기준은 앱의 `bucketByPeriod`/`billingPeriod` 다.** DB 에 '편한 집계 뷰' 를 다시 만들면
  멤버별 시작일(`app_settings.billing_start_*`)을 놓쳐 같은 버그가 재생산된다.
- ⚠️ `upsert(..., {onConflict:"…"})` 는 **그 이름의 유니크 제약이 DB 에 실재해야** 동작한다. 없으면 저장이 `dbErr` 토스트 한 줄로 끝나
  원인이 안 보인다 (현재 4곳 전부 대응 확인 — 대조표는 `docs/schema.md`).

### 검증 짝 (둘 다 통과해야 끝난 것)
- `node scripts/verify_rls.js` — anon 이 막혔는가 (기대: 가계부 401, 삭제된 뷰 404, 휴양림 200)
