...
> **2026-08-24 디자인 개편 직후 배포본 실측**: 데스크톱 DCL 301ms · FCP 300ms · **368KB** /
> 4G+CPU4배 DCL 1,027ms · FCP 872ms · **367KB** (요청 12개).
> **바이트 −16KB는 확정적이다**(결정적 값): 홈화면 아이콘을 골드 이모지 PNG에서 잉크 ₩ 획으로 바꿔 −23KB.
> ⚠️ **시간은 1회 측정이라 개선으로 읽지 말 것** — 위 경고대로 같은 파일도 CDN 편차만으로 크게 흔들린다.
> 위 표를 여전히 기준선으로 쓰되, 총 바이트 기대치만 368KB로 본다.
